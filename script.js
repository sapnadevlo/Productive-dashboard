function openFeatures() {
    ...
    allElems.forEach(function (elem) {
        elem.addEventListener('click', function () {
            ...
            // Fetch new quote when motivation card (id=2) opens
            if (elem.id === '2') {
                fetchQuote()
            }
        })
...
function todoList() {
    ...
    // index of task being edited; null when adding new task
    var editIndex = null
    ...
    let submitBtn = document.querySelector('.addTask form button')
    ...
    function renderTask() {
        ...
        currentTask.forEach(function (elem, idx) {
            sum = sum + `<div class="task">
                <div>
                    ...
                    <p class="details">${elem.details ? elem.details : ''}</p>
                </div>
                <div class="task-actions">
                    <button class="edit-btn" data-index="${idx}">Edit</button>
                    <button class="delete-btn" data-index="${idx}">Mark as Completed</button>
                </div>
            </div>`
        })
        ...
        document.querySelectorAll('.edit-btn').forEach(function (btn) {
            btn.addEventListener('click', function () {
                var idx = Number(btn.getAttribute('data-index'))
                var task = currentTask[idx]
                taskInput.value = task.task
                taskDetailsInput.value = task.details || ''
                taskCheckbox.checked = !!task.imp
                editIndex = idx
